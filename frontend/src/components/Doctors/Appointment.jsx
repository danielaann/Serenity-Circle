    ...
    return (
        <div className='w-[85%] mt-2 mx-2'>
            ...
            <div className='sm:ml-72 sm:pl-4 mt-4 font-medium text-gray-700'>
                ...
                <div className='flex gap-3 items-center mt-4'>
                    {
                        docSlots.length > 0 && docSlots.map((item, index) => (
                            <div onClick={()=>setSlotIndex(index)} key={index} className={`text-center py-6 min-w-16 rounded-full cursor-pointer ${slotIndex === index ? 'bg-accent': 'border border-gray-200'}`}>
                ...
                <div className='flex items-center overflow-x-auto scrollbar-hide mt-2 gap-3'>
                    {docSlots.length && docSlots[slotIndex].map((item,index)=>(
                        <p onClick={() => setSlotTime(item.time)} key={index} className={`text-sm font-light flex-shrink-0 px-5 py-2 rounded-full cursor-pointer ${item.time === slotTime ? 'bg-secondary': 'text-gray-400 border border-gray-300'}`}>
